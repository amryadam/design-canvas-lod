...
own, so the end of one does not free the other. A hold with no start, no end
and no frame of movement for 5 s is taken as lost, so a drag that never
reports its end cannot stop the budget, and a gesture that keeps moving is
never cut short.
