...
const dcZoom = { scale: 1, subs: new Set() };
function dcSetZoom(scale) {
  dcZoom.scale = scale;
  dcZoom.subs.forEach((f) => { try { f(scale); } catch {} });
}
...
function DCLazyFrame({ src, title, width, height, eager = false, margin = 600, href, thumb }) {
  ...
  React.useEffect(() => {
    ...
    const decide = () => {
      ...
      if (want !== isLive) { isLive = want; setLive(want); }
    };
    const schedule = () => { clearTimeout(timer); timer = setTimeout(decide, DC.settleMs); };
    decide();
    dcZoom.subs.add(schedule);
    // IntersectionObserver is silent in hidden documents; poll as a fallback.
    const io = new IntersectionObserver(schedule, { rootMargin: `${margin}px` });
    io.observe(ref.current);
    const poll = setInterval(decide, 500);
    document.addEventListener('visibilitychange', schedule);
    return () => {
      clearTimeout(timer); dcZoom.subs.delete(schedule); io.disconnect(); clearInterval(poll);
