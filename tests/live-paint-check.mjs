...
// (idle rAF under 50 fps, the tab is not visible, or nothing could be judged).
...
try {
  ...
  const judged = cards.map((cd, i) => ({ ...cd, restInk: rest[i].ink }))
    .filter((cd, i) => rest[i].n > 0 && rest[i].ink >= MIN_REST_INK);
  ...
  for (const leg of legs) {
    ...
    for (let s = leg.from; s < leg.to; s++) {
      ...
      judged.forEach((cd) => {
        ...
        if (v.n === 0) return;                      // off screen, or too small to judge
  ...
  console.log(`LIVE PAINT CHECK ${TAG}: ${blanks.length ? 'FAIL' : 'PASS'} — ${blanks.length} of ${rows.length} live-card readings were blank during the gesture (worst ink=${worst.ink} on ${worst.id}, rest ${worst.restInk})`);
  if (CONTROL) {
